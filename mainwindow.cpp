    ...
    ui(new Ui::MainWindow)
{
    ...
    mBlock = new Block();//开个棋盘
    mBlock->Create();//初始化棋盘

    setFixedSize(mBlock->mCol*BLOCK_WIDTH,mBlock->mRow*BLOCK_HEIGHT+BOARD_HEIGHT);//设置窗口尺寸
...
void MainWindow::paintEvent(QPaintEvent *event)
{
    ...
    if(background_c==1){
        bmpblocks.load(":/res/new.jpg");
        bmpblast.load(":/res/mine1.jpg");
        bmpflagerror.load(":/res/wrong.jpg");
    }
    //绘制雷区
...
{
    QMessageBox message(QMessageBox::Information, "information", "Wriiten by Lostb --2023.8");
    message.exec();
}
...
void MainWindow::on_actionrank_triggered()
{
    ...
    back_ground->exec();
    if(back_ground->choice==0){
        background_c=0;
        qDebug()<<"0";
    }
    else if(back_ground->choice==1){
        background_c=1;
        qDebug()<<"1";
    }
    else if(back_ground->choice==2){
        background_c=2;
        qDebug()<<"2";
    }
